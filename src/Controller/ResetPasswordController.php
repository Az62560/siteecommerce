...
class ResetPasswordController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($request->get('email')) {
              ...
              if ($user) {
                //Envoyer en bdd la demande de reset_password avec user, token, createdAt.
                ...
                //Envoyer un email avec un lien pour mettre à jour son mot de passe
                $url = $this->generateUrl('update_password', [
                    'token' => $reset_password->getToken()
                ]);

            
                $content = "Bonjour ".$user->getFirstname()."<br>Vous avez demandé à réinitialiser votre mot de passe sur le site E-commerce.<br><br>";
                ...
                $this->addFlash('notice', 'Vous allez recevoir un email avec la procédure à suivre.');
    ...
    public function update(Request $request, $token, UserPasswordHasherInterface $encoder): Response
    {
        ...
        $createdAt = $reset_password->getCreatedAt();
        $timestampNow = $now->getTimestamp();
        $timestampCreatedAt = $createdAt->getTimestamp();
        $timeDifferenceInHours = ($timestampNow - $timestampCreatedAt) / 3600; // Convertir la différence en heures
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            //Encodage du nouveau mot de passer
